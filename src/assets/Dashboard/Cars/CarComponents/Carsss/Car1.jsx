...
const Car1 = (propse) => {
  ...
  const [Ids, setIds] = useState({ carId: propse.carid, detailsId: propse.detailsid });
  const [cardata, setdata] = useState(propse.car._id);
  const navigation = useNavigate("");

  const navigate = () => {
    navigation('cardetails', { state: { id: cardata } })
    console.log("Car Das is passed",cardata);
  }
  ...
  async function addtoFavourite() {
    ...
    const URL = `http://localhost:5000/api/favorite/addFavorite`;
    const Token = localStorage.getItem("Token");
    console.log("Token", Token);

    try {
      const favCarResponse = await axios.post(URL, {
        'car': propse.car._id,
      }, {
        headers: {
  ...
  async function removeFav() {
    const URL = `http://localhost:5000/api/favorite/removeFavorite`;
    const Token = localStorage.getItem("Token");
    try {
  ...
  if (location.pathname == '/showroomowner/bookedcars') {
    isRentNow = false;
  ...
  let turn = true;
  return (
    <div className='w-[272px] min-fit-h-[369px] bg-[white] text-black m-[10px] border-[1px] border-[#dadada]  rounded-lg flex flex-col justify gap-[10px] p-[20px]'>
      <div id="logo" className='flex flex-row justify-between  items-center'><b>{propse?.car?.title ? propse?.car?.title : propse.bookedCar?.title}</b>
        <svg onClick={() => {
          if (turn == true) {
            addtoFavourite();
            setcolor("red");
      ...
      <img className=" w-[253px] h-[166px] object-contain border-[1px] border-black rounded-sm " src={localhost() + (propse.car?.pictures?.[0] ? propse.car?.pictures?.[0] : propse.bookedCar?.pictures[0])}
        alt="" />
      <div id="carinfo" className='flex flex-row items-center gap-[1vmin]'>
        <div className='flex flex-row items-center gap-[0.5vw] justify-evenly w-[100%]'>
          ...
          <p className='text-[#90A3BF] text-[12px] capitalize'>{propse.car.fuelType ? propse.car.fuelType : propse.bookedCar?.fuelType}</p>
        </div>
        ...
        <div className='flex flex-row items-center gap-[0.5vw] justify-evenly w-[100%]'>
          ...
          </svg><p className='text-[#90A3BF] text-[12px] capitalize'>{propse.car.feature ?? propse.bookedCar?.feature}</p>
        </div>
        <div className='flex flex-row items-center  gap-[0.5vw] justify-evenly w-[100%]'>
          <svg className="w-[16px] h-[15px]" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          </svg><p className='text-[#90A3BF] text-[12px] capitalize'>{propse?.car?.driverType ? propse.car.driverType : propse.bookedCar?.driverType}</p>
        </div>
      </div>
      <div className='flex flex-col gap-[1vmin] w-[100%]'>
        <p className='font-bold w-[100%] text-[12px] inline-block'>{propse.car.discountedPrice ? propse.car.discountedPrice : propse.bookedCar?.discountedPrice}/day</p>
        { location.pathname == '/showroomowner/showroomcars' ? <>
          <div className="w-[100%] flex flex-row px-[1vmin] justify-between">
            <button onClick={() => {
              navigation("rentcar",{state: {id: Ids}});
              localStorage.setItem("UpdateCar",true);
            }} className='#FC5C00  py-[1vmin] px-[1.4vmin] text-sm w-[60px]'>Edit</button>
            <button onClick={() => {
              navigation('cardetails', { state: { id: Ids } })
        ...
        {location.pathname == '/showroomowner/bookedcars' && filter == 'pending' ?
          <div className="w-[100%] flex flex-row px-[1vmin] justify-between">
            <button onClick={() => {
              cancelbooking();
