...
const Car2 = (propse) => {
  ...
  const [display, setdisplay] = useState(propse.picture);
  ...
  const [strokecolor, setstrokecolor] = useState('#bdbdbd');


  const navigation = useNavigate("");

  const navigate = () => {
    navigation('cardetails', { state: { id: propse.car.car._id, } })
  }


  return (
    <div className='w-[23%] max-h-[80%] border-[1px]  border-[#dadada]  text-black bg-[#ffffff] rounded-lg flex flex-col justify-between p-[10px]'>
      <div id="logo" className='flex flex-row justify-between  items-center'><b>{propse.car.car.title}</b>
        <svg onClick={() => {
        ...
        }} className="w-[3vmin] h-[4vmin]" id='svg' viewBox="0 0 22 20" fill={heartcolor} xmlns="http://www.w3.org/2000/svg">
          <path d="M11.62 18.8101C11.28 18.9301 10.72 18.9301 10.38 18.8101C7.48 17.8201 1 13.6901 1 6.6901C1 3.6001 3.49 1.1001 6.56 1.1001C8.38 1.1001 9.99 1.9801 11 3.3401C12.01 1.9801 13.63 1.1001 15.44 1.1001C18.51 1.1001 21 3.6001 21 6.6901C21 13.6901 14.52 17.8201 11.62 18.8101Z" stroke="red" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
        </svg>
      </div>
      <img className="py-[4.7vmin] w-[40vmin] h-[40vmin]" src={'http://localhost:5000/' + display} alt="" />
      <div id="carinfo" className='flex flex-row items-center gap-[1vmin]'>
        <div className='flex flex-row items-center justify-evenly w-[100%]'>
          ...
          </svg><p className='text-[#90A3BF] text-[12px] capitalize'>{propse.car.car.fuelType}</p>
        </div>
        ...
        <div className='flex flex-row items-center justify-evenly w-[100%]'>
          ...
          </svg><p className='text-[#90A3BF] text-[12px] capitalize'>{propse.car.car.feature}</p>
        </div>
        ...
        <div className='flex flex-row items-center justify-evenly w-[100%]'>
          ...
          </svg><p className='text-[#90A3BF] text-[12px]'>{propse.car.car.driverType}</p>
        </div>
      </div>
      <div className='flex flex-col gap-[1vmin]  w-[100%]'>
        <p className='font-bold w-[100%] inline-block '> {propse.car.car.discountedPrice}/day</p>

        <button onClick={() => { navigate(); }} className='py-[1vmin] px-[1.4vmin] bg-[#FF5C1E] rounded-[0.5vmin] text-white text-[12px] font-semibold w-[100%]'> <Link to='cardetails'> Rent Now</Link></button>
      </div>
    </div>
  )
}
